...
function openPaymentModal(serviceName, servicePrice) {
  ...
  if (serviceAmountSpan) serviceAmountSpan.textContent = `$${servicePrice} USD`;
  ...
  if (fillTestDataBtn) {
    fillTestDataBtn.addEventListener('click', function() {
      ...
      if (!isVisible) {
        ...
        this.innerHTML = '<i class="fas fa-times"></i><span>Ocultar Datos de Prueba</span>';
        ...
        this.innerHTML = '<i class="fas fa-flask"></i><span>Rellenar con Datos de Prueba</span>';
      }
    });
  }
}
  ...
  modalPaymentForm.addEventListener('submit', async function(e) {
    ...
    try {
      ...
      if (result.success) {
        ...
        showModalPaymentResponse('success', result.message || 'Pago procesado exitosamente');
        ...
        showModalPaymentResponse('error', result.message || 'Error al procesar el pago');
      }
    } catch (error) {
      ...
      showModalPaymentResponse('error', 'Error de conexión. Por favor, intenta nuevamente.');
    } finally {
      // Hide loading state
      hideLoadingAnimation();
